...
for col in X_all.columns:
    ...
    for name, group in groups:
        ...
        X_all['cnt'+col].iloc[group.index] = count
...
for col in X_all.columns[1:6]:
    ...
    for name, group in groups:
        ...
        for rsrc, grp in grps:
            X_all['Duse'+col].iloc[grp.index] = float(len(grp.index)) / float(len(group.index) )
...
for col in X_all.columns[0:1]:
    ...
    for name, group in groups:
        X_all['Mdeps'+col].loc[group.index] = len(group[col].unique()) 
...
X_train, X_cv, y_train, y_cv = model_selection.train_test_split(X, y, test_size=0.5, random_state=SEED)
