                ...
                for MIN_SAMPLES_SPLIT in range(2, 3):
                    ...
                    for c in range(CV_FOLDS):
                        ...
                        for bird in range(num_species):
                            ...
                            rfr = RandomForestRegressor(n_estimators=N_ESTIMATORS, max_features=MAX_FEATURES, min_samples_split=MIN_SAMPLES_SPLIT,
                                                        random_state=rs * 100, verbose=0)
                    ...
                    ResultDf = pd.DataFrame(RESULT,
                                            columns=['ID', 'NUM_FEATURES', 'N_ESTIMATORS', 'MAX_FEATURES', 'MIN_SAMPLES_SPLIT', 'CV_FOLDS', 'AUC'])
                    ...
                    if (auc > 0.93):
                        ...
                        SubmissionDf.to_csv(
                            subm_folder + 'rfr_osl_cv15_freq_5' + str(ID) + '_' + str(NUM_FEATURES) + '_' + str(N_ESTIMATORS) + '_' + str(
                                MAX_FEATURES) + '_' + str(MIN_SAMPLES_SPLIT) + '_' + str(CV_FOLDS) + '.csv', index=False)