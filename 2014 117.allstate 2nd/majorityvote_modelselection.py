...
def majority_vote(baseline, model_predictions):
    ...
    preds = baseline + 0
    preds[prmax] = np.argmax(prcnt[prmax], axis=1)
    return preds
...
def make_ptscores(y_true, y_pred, y_base, pt, vmask):
    ...
    return [
        np.mean(vmask[pt == ipt] & (y_true[pt == ipt] == y_pred[pt == ipt])) - np.mean(vmask[pt == ipt] & (y_true[pt == ipt] == y_base[pt == ipt]))
...
if __name__ == '__main__':
    ...
    # print
    # "Majority vote using %i models, selecting %i\n" % (N, NS)
    # initialize data
    data, test, con, cat, extra, conf, conf_f, encoders = prepare_data()
    data = data[data.shopping_pt >= include_from_pt]
    # print
    # "Including from shopping_pt #%i\n" % data.shopping_pt.min(),
    ...
    for n, m, l in params:
        ...
        for ifold, (itr, icv) in enumerate(model_selection.KFold(kfold).split(X,y)):
            if submit:
                # just a lame way to re-using the same code for fitting & selecting when submitting :)
                itr = np.ones(y.shape, dtype=bool)
                icv = -itr
            ...
            rftscores.sort(reverse=1);
            selected = [x[1] for x in rftscores[:NS]]
            ...
            # print
            str(ifold) + ",a " + " ".join(["%.5f" % pts for pts in ptscore]) + " %.2f" % tn.dot(ptscore[1:])
            ...
            # print
            str(ifold) + ",s " + " ".join(["%.5f" % pts for pts in ptscore]) + " %.2f" % tn.dot(ptscore[1:])
        ...
        print(
        " ".join(["%.5f" % pts for pts in np.std(ptscores[(m, l)], axis=0)]))
    ...
    if submit:
        ...
        test['pG'] = majority_vote(test.G, allpreds[:, selected]);
        print("done")

        # Fix state law products, then concatenate to string
        stateFix(encoders, test, ['C', 'D', 'pG'], 1)
        test['plan'] = concat(test, ['A', 'B', 'C', 'D', 'E', 'F', 'pG'])
        test['plan'].to_csv('submission\\majority_rfs%i_%i.%i_shuffle_GAfix_%iof%iof%i.csv' % (n, m, l, NS / 2 + 1, NS, N), header=1)
