...
def stateFix(encoders,df,c=['C','D','G'],verbose=False):
    ...
    ifix = iGA&(df[c[0]]==1); df.loc[ifix,c[0]] = 2; nga1 = np.sum(ifix) #C
    ifix = iGA&(df[c[1]]==1); df.loc[ifix,c[1]] = 2; nga2 = np.sum(ifix) #D
    # FL
    iFL = df.state == encoders['state'].transform(['FL'])[0]
    ifix = iFL&(df[c[2]]<=2); df.loc[ifix,c[2]] = 3; nfl1 = np.sum(ifix) #G
    # OH
    iOH = df.state == encoders['state'].transform(['OH'])[0]
    ifix = iOH&(df[c[2]]==1); df.loc[ifix,c[2]] = 2; noh1 = np.sum(ifix) #G
    # ND
    iND = df.state == encoders['state'].transform(['ND'])[0]
    ifix = iND&(df[c[2]]!=2); df.loc[ifix,c[2]] = 2; nnd1 = np.sum(ifix) #G
    # SD
    iSD = df.state == encoders['state'].transform(['SD'])[0]
    ifix = iSD&(df[c[2]]!=2); df.loc[ifix,c[2]] = 2; nsd1 = np.sum(ifix) #G
    if verbose:
        print("Fixed state law products. GA1:%i GA2:%i FL1:%i OH1:%i ND1:%i SD1:%i" %(nga1, nga2, nfl1, noh1, nnd1, nsd1))
...
def expval(df,col,y,tfilter):
    ...
    tmp.loc[tfilter,'cnt'] -= 1                                              # reduce count for train set
    tmp.loc[tfilter,'csm'] -= df.loc[tfilter,y]                               # remove current value
    tmp['exp'] = ((tmp.csm+ pb*15) / (tmp.cnt+ 15)).fillna(pb)              # calculate mean including kn-extra 'average' samples
    np.random.seed(1)
    tmp.loc[tfilter,'exp'] *= 1+.3*(np.random.rand(len(tmp[tfilter]))-.5) # add some random noise to the train set
    return tmp.exp

def prepare_data(shuffle=True):
    alltest = pd.read_csv('test_v2.csv')
    ...
    data.loc[sub,'location'] = data.state[sub]; print('%.5f' % sub.mean())
    ...
    if shuffle:
        ...
        data.sort_values(['rand','shopping_pt'],inplace=True); print("DONE!")
