...
def main():
    ...
    print
    time.asctime(time.localtime(time.time())) + ' ---  Program started'
    ...
    print
    time.asctime(time.localtime(time.time())) + ' ---  Calculating sentiment scores...'
    scoredReviews = calcReviewSentimentScore(termScores, jsonData)
    # print scoredReviews
    print
    time.asctime(time.localtime(time.time())) + ' ---  Writing csv output...'
    writeCSV(scoredReviews)
    execution_time = time.time() - start_time
    print
    time.asctime(time.localtime(time.time())) + ' ---  Program completed.  Execution time (mins): ' + str(execution_time / 60)
