...
def run_from_step(step):
    ...
    # else:
    #     print "\tLoading files generated from previous."
    #     potential_duplicate_groups = pickle.load(
    ...
    # else:
    #     real_duplicate_groups = pickle.load(
    #             open(serialization_dir + '_' + real_duplicate_groups_file, "rb"))
    #     similarity_score_dict = pickle.load(
    ...
    while iter_num > 0:
        find_closure(authors_duplicates_dict)
        refine_result(cannot_links, authors_duplicates_dict, name_instance_dict, id_name_dict, name_statistics, similarity_score_dict, metapaths,
                      True)
        iter_num -= 1
    # find_closure(authors_duplicates_dict)
