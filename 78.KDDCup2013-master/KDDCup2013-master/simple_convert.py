...
def generate_new_author_names():
    author_fn = 'data/Author_refined_simple2.csv'
    paper_author_fn = 'data/PaperAuthor_refined_simple1.csv'
    if not os.path.isfile(author_fn):
        ...
        with open("data/Author.csv", 'r', encoding='utf-8') as author_file:
            f = open(author_fn, 'w',encoding='utf-8')
            for line in author_file:
                ...
                if len(tokens) > 1:
                    if tokens[1] in done:
                        ...
                        clean = remove_noise(tokens[1]).decode()
                        done[tokens[1]] = clean
                        tokens[1] = clean
                    # f.write(','.join(tokens).replace('\xd8','').replace('\xb4','' ).replace('\xb8',''))
                    f.write(','.join(tokens))
                else:
                    f.write(line)
    if not os.path.isfile(paper_author_fn):
        ...
        with open("data/PaperAuthor.csv", 'r', encoding='utf-8') as pa_file:
            f = open(paper_author_fn, 'w',encoding='utf-8')
            for line in pa_file:
                tokens = line.split(',')
                if len(tokens) > 2:
                    if tokens[2] in done:
                        ...
                        clean = remove_noise(tokens[2]).decode()
                        done[tokens[2]] = clean
                        tokens[2] = clean
                    f.write(','.join(tokens))
                else:
